...
/
/#### 哦哄，各种声明，这里是FATFS是源码，不用进行任何修改，哦轰轰轰
/----------------------------------------------------------------------------*/


#include "ff.h"			/* Declarations of FatFs API  FatF的 API声明*/
#include "diskio.h"		/* Declarations of disk I/O functions   磁盘 I/O 函数的声明*/


/*--------------------------------------------------------------------------

   Module Private Definitions	模块私有定义

---------------------------------------------------------------------------*/

#if _FATFS != 64180	/* Revision ID 		修改ID*/
...
#if _FS_REENTRANT
#if _USE_LFN == 1
#error Static LFN work area cannot be used at thread-safe configuration  // 静态LFN工作区域不能在线程安全配置中使用
#endif
...
/* Definitions of sector size  扇区大小定义*/
#if (_MAX_SS < _MIN_SS) || (_MAX_SS != 512 && _MAX_SS != 1024 && _MAX_SS != 2048 && _MAX_SS != 4096) || (_MIN_SS != 512 && _MIN_SS != 1024 && _MIN_SS != 2048 && _MIN_SS != 4096)
#error Wrong sector size configuration
#endif
#if _MAX_SS == _MIN_SS
#define	SS(fs)	((UINT)_MAX_SS)	/* Fixed sector size  固定扇区大小*/
#else
#define	SS(fs)	((fs)->ssize)	/* Variable sector size 可变扇区大小*/
#endif
