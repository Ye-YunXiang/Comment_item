 ...
 * 
 * 生成和使用RFC 4122中指定的uid
 * 该模块基于RFC4122中的规范实现了第4板uuid的生成，以及在字符串的二进制表示之间进行转换的例程。
 */
#ifndef OC_UUID_H
#define OC_UUID_H
...
/**
 * The length of a UUID string.  UUID字符串的长度
 *
 * This is the length of UUID string as specified by RFC 4122.  这是RFC_4122指定的UUID字符串的长度。
 * @see oc_uuid_to_str
 */
...
/**
 * Convert a UUID string representation to a 128-bit oc_uuid_t  将UUID字符串表示形式转换为128位oc_uuid_t
 *
 ...
 *       to zero.
 * 有一个不符合RFC 4122的特殊情况，如果str的第一个字符是“*”，纳闷oc_uuid_td的第一个字节将被设置位“*”（0X2A），其他字节将被设置为0。
 * 
 *
 * Example  例子
 * ```
 ...
 * ```
 * @param[in] str the UUID string    UUID字符串
 * @param[out] uuid the oc_uuid_t to hold the UUID bits.  用oc_uuid来保存UUID
 */
