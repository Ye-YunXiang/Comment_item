 ...
 * Description: Slave management functions.     Slave管理功能
 */
...
void slave_list_init() {            // 从设备列表初始化
    for(uint8_t i = 0; i < MAX_SLAVES; i++) {
        slave_clear_slot(i);                // 从设备清除存储槽
    }
}

uint8_t slave_list_available() {        // 从设备列表有效的
    for(uint8_t i = 0; i < MAX_SLAVES; i++)
        if(slaves[i].addr == 0x00)
            return i + 1; // Valid addresses start from 1       有效地址从1开始
    return 0;
}
...
void slave_clear_slot(uint8_t index) {      // 清除存储槽
    slaves[index].addr = 0x00;              // 累加器清零
    memset(slaves[index].uid, 0, UID_SIZE);
    slaves[index].ping_count = 0xFF;        // ping次数计数
    slaves[index].ping_error = 0;           // ping错误次数
}

void slave_ping_error(uint8_t addr) {
    if(++(slaves[--addr].ping_error) == MAX_PING_ERROR) // Convert to index 转换为索引
        slave_clear_slot(addr); // Pass the index   传送索引
}

void slave_set_ping_period(uint8_t addr, uint8_t ticks) {
    if(ticks) {
        slaves[--addr].ping_count = ticks; // Convert from addr to index      地址转换为索引
    ...
    for(uint8_t i = 0; i < MAX_SLAVES; i++)
        if( (slaves[i].addr != 0x00) && (slaves[i].ping_count > 0) )
            if(--slaves[i].ping_count == 0)
                ping_flag = true; // At least one slave needs to be pinged      至少Ping同一个节点
}
